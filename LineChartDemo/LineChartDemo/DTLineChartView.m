...
@implementation DTLineChartView
// 初始化
...
- (NSMutableArray *)pointArray {
    if (_pointArray == nil) {
        _pointArray = [NSMutableArray arrayWithObjects:[DTLineChartPoint pointWithX:1 andY:1], [DTLineChartPoint pointWithX:2 andY:2], [DTLineChartPoint pointWithX:3 andY:1.5], [DTLineChartPoint pointWithX:4 andY:2], [DTLineChartPoint pointWithX:5 andY:4], [DTLineChartPoint pointWithX:6 andY:1], [DTLineChartPoint pointWithX:7 andY:2], nil];
    }
    return _pointArray;
}
...
- (void)setupLineChartLayerAppearance {
    ...
    [self.pointArray enumerateObjectsUsingBlock:^(DTLineChartPoint * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        // 折线
        if (idx == 0) {
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (self.tapButton == nil) { // 首次添加标签（避免多次创建和计算）
        DTLineChartPoint *point = self.pointArray[self.pointArray.count-1];
        CGRect tapButtonFrame = CGRectMake(self.xAxisSpacing * 0.5 + (point.x - 1) * self.xAxisSpacing + 8, self.bounds.size.height - kPadding - point.y * self.yAxisSpacing - 34, 30, 30);
